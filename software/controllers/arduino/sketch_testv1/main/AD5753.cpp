...
#if SERIAL_DEBUG and AD5753_DEBUG
    #define SIMPLE_DEBUG(x) Serial.println((x));
#else
...
exit_code transfer_ad5753_spi(uint8_t* buf, uint8_t* retbuf)
{
    ...
    #if SERIAL_DEBUG and SPI_DEBUG
        Serial.print("SPI - AD - Received: ");
        for(int i; i<4; i++){
            Serial.print(retbuf[i], HEX);
            Serial.print('-');
        }
        Serial.print("(");
        Serial.print(ad5753_compute_crc8(retbuf, 3), HEX);
        Serial.print(")/");
        if (crc_check) {
            Serial.print("OK");
        } else {
...
exit_code ad5753_reg_read(spi_device dev, uint8_t reg_address, uint16_t *read_data)
{
    ...
    *read_data = (retbuf[1] << 8) | retbuf[2];
    ...
    bool fault = ((retbuf[0] >> 5) & 0x1);  // get fault pin status
        ...
        Serial.print(reg_address, HEX);
        Serial.print(": (ADDR,FAULT,SPIFAULT) ");
        Serial.print(*read_data, HEX);
        Serial.print("/");
        Serial.print(fault, DEC);
        Serial.print("/");
        Serial.print(spi_fault, DEC);
        Serial.println();
    #endif
    ...
    ret |= fault * AD5753_FAULT;
        ...
        uint16_t _diag_data;
        if (fault > 0 &&
            reg_address != AD5753_REG_DIGITAL_DIAG_RESULTS &&
            reg_address != AD5753_REG_STATUS &&
            reg_address != AD5753_REG_ANALOG_DIAG_RESULTS)
        {
            Serial.println("AD5753 FAULT...");
            ad5753_reg_read32(dev, AD5753_REG_STATUS, &_status_data);
            Serial.print("STATUS: ");
            Serial.println(_status_data, HEX);
            ad5753_reg_read(dev, AD5753_REG_DIGITAL_DIAG_RESULTS, &_diag_data);
            Serial.print("DIGITAL_DIAG_RESULTS: ");
            Serial.println(_diag_data, HEX);
            ad5753_reg_read(dev, AD5753_REG_ANALOG_DIAG_RESULTS, &_diag_data);
                                ...
                                uint16_t data)
{
    ...
    if (ret > 0) {
        #if SERIAL_DEBUG and REG_DEBUG
            Serial.print("Aborted write due to BAD READ: ");
            Serial.print(reg_data, HEX);
            Serial.print(", excode: 0x");
...
exit_code ad5753_adc_single_conversion(spi_device dev, uint8_t ip_select, uint16_t* adc_data)
{
    uint32_t status_reg;
    exit_code ret = SUCCESS;
    ...
    ret |= ad5753_reg_write_mask(dev,
                          ...
                          (AD5753_ADC_CONFIG_SEQUENCE_COMMAND_MSK |
                           AD5753_ADC_CONFIG_ADC_IP_SELECT_MSK),
                          (AD5753_ADC_CONFIG_SEQUENCE_COMMAND_MODE(0x4)|  //Single conversion
                           AD5753_ADC_CONFIG_ADC_IP_SELECT_MODE(ip_select))
                         );

    EARLY_RETURN(ret)

    do {
        ret |= ad5753_reg_read32(dev, AD5753_REG_STATUS, &status_reg);
    } while (!ret && (status_reg & AD5753_STATUS_ADC_BUSY_MSK));

    *adc_data = status_reg & AD5753_STATUS_ADC_DATA_MSK;

    #if SERIAL_DEBUG and ADC_DEBUG
        Serial.print("ADC(");
        Serial.print(ip_select, HEX);
        Serial.print("): ")
        Serial.print(*adc_data, DEC);
        Serial.print("/");
        Serial.print(0xFFF, DEC);
        Serial.println();
    #endif
...
exit_code ad5753_read_adc1(spi_device dev, uint16_t* adc_data)
{
    return ad5753_adc_single_conversion(dev, 0b10001, adc_data);
}

exit_code ad5753_read_adc2(spi_device dev, uint16_t* adc_data)
{
    return ad5753_adc_single_conversion(dev, 0b01100, adc_data);
}
