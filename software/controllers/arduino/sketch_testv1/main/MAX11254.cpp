...
exit_code max11254_reg_write (spi_device dev, uint8_t reg_address, uint8_t* reg_value, uint8_t reg_size)
{
    ...
    ss_deselect(dev);
        ...
        Serial.print("SPI - MAX - Sending:  ");
        Serial.print(MAX11254_REG_WRITE(reg_address), HEX);
        Serial.print('-');
        for(int i=0; i<reg_size; i++){
            Serial.print(reg_value[i], HEX);
            Serial.print('-');
        }
        Serial.println();
    #endif
...
exit_code max11254_reg_read (spi_device dev, uint8_t reg_address, uint8_t* reg_value, uint8_t reg_size)
{
    ...
    #if SERIAL_DEBUG and SPI_DEBUG
        Serial.print("SPI - MAX - Sending:  ");
        Serial.println(MAX11254_REG_READ(reg_address), HEX);
        Serial.print("SPI - MAX - Received: ");
        for(int i=0; i<reg_size; i++){
            Serial.print(reg_value[i], HEX);
            Serial.print('-');
        }
